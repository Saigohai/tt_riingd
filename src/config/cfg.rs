...
use crate::{
    config::fan_curve::Point, drivers::registry::Registry, effects::effect_runner::EffectRunner,
};
...
impl ControllerCfg {
    ...
    pub fn get_fans_count(&self) -> u8 {
        match self {
            ControllerCfg::RiingQuad { fans, .. } => fans.len().try_into().unwrap(),
        }
    }
}
...
impl ConfigManager {
    ...
    pub async fn ensure_mappings(config: &mut Config) -> Result<()> {
        ...
        let (m, cm, em) = config
            ...
            .iter_mut()
            // .enumerate()
            .filter(|controller| controller.get_id().starts_with("autogenerated"))
            .fold(
                (
                    MappingCfg::dummy(),
